...
# ============================================================================
# NOVOS ENDPOINTS - FLUXO CONSULTIVO RAG
# ============================================================================

@etp_dynamic_bp.route('/review-requirements', methods=['POST'])
@limiter.limit("30 per minute")
@cross_origin()
def review_requirements():
    """
    Endpoint para revisão iterativa de requisitos.
    Suporta comandos em PT-BR: remover, trocar, manter apenas, etc.
    """
    try:
        _ensure_initialized()
        ...
        if not session_id or not user_message:
            return jsonify({
                'success': False,
                'error': 'session_id e user_message são obrigatórios'
            }), 400
        ...
        # Obter requisitos atuais
        current_requirements = session.get_requirements()
        
        if not current_requirements:
            return jsonify({
                'success': False,
                'error': 'Nenhum requisito encontrado na sessão'
            }), 400
        
        # Interpretar comando do usuário
        from domain.usecase.etp.requirements_interpreter import parse_update_command
        
        cmd = parse_update_command(user_message, current_requirements)
        
        if cmd['intent'] == 'unclear':
            return jsonify({
                'success': False,
                'message': cmd['message'],
                'hint': 'Exemplos: "remova 2 e 4", "troque o 3", "mantenha apenas 1, 2 e 5"'
            }), 400
        ...
        if cmd['intent'] == 'remove':
            # Remover requisitos
            session.remove_requirements(cmd['items'])
            changed_ids = cmd['items']
            message = f"Removi os requisitos {', '.join(changed_ids)}. Está adequado agora?"
        
        elif cmd['intent'] == 'keep_only':
            # Manter apenas os especificados
            removed = [req['id'] for req in current_requirements if req['id'] not in cmd['items']]
            session.keep_only_requirements(cmd['items'])
            changed_ids = removed
            message = f"Mantive apenas os requisitos {', '.join(cmd['items'])}. Está adequado agora?"
        
        elif cmd['intent'] == 'edit':
            # Trocar/editar requisitos
            for req_id in cmd['items']:
                # Gerar novo requisito via RAG
                necessity = session.necessity or ""
                rag_results = search_requirements("generic", necessity, k=3)
                
                # Usar IA para gerar novo requisito
                new_req_text = _generate_single_requirement(necessity, rag_results, req_id)
                session.update_requirement(req_id, new_text=new_req_text)
            
            changed_ids = cmd['items']
            message = f"Substituí os requisitos {', '.join(changed_ids)}. Está adequado agora?"
        
        elif cmd['intent'] == 'confirm':
            # Confirmar requisitos
            session.conversation_stage = 'requirements_confirmed'
            db.session.commit()
            
            return jsonify({
                'success': True,
                'requirements': session.get_requirements(),
                'message': 'Requisitos confirmados! Vou verificar se preciso de mais informações sobre o caminho de contratação.',
                'next_action': 'check_ambiguity'
            })
        
        elif cmd['intent'] == 'restart_necessity':
            # Reiniciar necessidade
            session.necessity = None
            session.requirements_json = None
            session.conversation_stage = 'collect_need'
            db.session.commit()
            
            return jsonify({
                'success': True,
                'message': 'Certo, vamos recomeçar. Qual é a necessidade de contratação?',
                'next_action': 'collect_necessity'
            })
        ...
        logger.error(f"Erro em review_requirements: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Erro ao revisar requisitos: {str(e)}'
        }), 500


@etp_dynamic_bp.route('/options-if-ambiguous', methods=['POST'])
@limiter.limit("30 per minute")
@cross_origin()
def options_if_ambiguous():
    """
    Endpoint para sugerir opções de caminho quando a necessidade é ambígua.
    Exemplo: compra vs locação vs comodato.
    """
    try:
        data = request.get_json()
        session_id = data.get('session_id')
        ...
        if not advisor.is_ambiguous(necessity, requirements):
            # Não é ambíguo, pular para próximo passo
            return jsonify({
                'success': True,
                'ambiguous': False,
                'message': 'Caminho de contratação já está claro.',
                'next_action': 'continue_pca'
            })
        
        # É ambíguo, sugerir opções
        options = advisor.suggest_options(necessity, requirements)
        response = advisor.format_options_for_response(options)
        
        # Atualizar estágio da conversa
        session.conversation_stage = 'options'
        db.session.commit()
        ...
        logger.error(f"Erro em options_if_ambiguous: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Erro ao verificar ambiguidade: {str(e)}'
        }), 500


@etp_dynamic_bp.route('/pick-option', methods=['POST'])
@limiter.limit("30 per minute")
@cross_origin()
def pick_option():
    """
    Endpoint para registrar a opção escolhida pelo usuário.
    """
    try:
        data = request.get_json()
        session_id = data.get('session_id')
        ...
        if not session_id or not option_id:
            return jsonify({
                'success': False,
                'error': 'session_id e option_id são obrigatórios'
            }), 400
        ...
        # Mapear option_id para procurement_path
        path_map = {
            'opt_compra': 'compra',
            'opt_locacao': 'locacao',
            'opt_comodato': 'comodato',
            'opt_servico': 'servico'
        }
        
        procurement_path = path_map.get(option_id)
        
        if not procurement_path:
            return jsonify({
                'success': False,
                'error': 'option_id inválido'
            }), 400
        
        # Salvar escolha
        session.procurement_path = procurement_path
        session.conversation_stage = 'path_selected'
        db.session.commit()
        ...
        path_labels = {
            ...
            'comodato': 'Comodato (Cessão Gratuita)',
        ...
        return jsonify({
            ...
            'ok': True,
            'session_id': session_id,
            'chosen': procurement_path,
            'message': f'Perfeito! Vamos prosseguir com {path_labels[procurement_path]}.',
            'next_action': 'continue_pca'
        })
    
    except Exception as e:
        logger.error(f"Erro em pick_option: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Erro ao registrar opção: {str(e)}'
        }), 500


def _generate_single_requirement(necessity: str, rag_results: list, req_id: str) -> str:
    """
    Gera um único requisito usando IA e RAG.
    """
    try:
        _ensure_initialized()
        ...
        E nos seguintes exemplos similares:
        {json.dumps(rag_results[:2], indent=2, ensure_ascii=False)}
        
        Gere UM requisito técnico específico e objetivo para esta contratação.
        
        Retorne APENAS o texto do requisito, sem numeração ou formatação adicional.
        """
        ...
        logger.error(f"Erro ao gerar requisito: {str(e)}")
        return f"Requisito técnico relacionado a {necessity}"
